...
| F004 | Verify personal_notes in public mirror (M4) | Infrastructure | Medium | v1.0.8 | Low | Medium | 💡 Idea | personal_notes written to public_tastings — confirm intent and restrict if unintentional |
...
| F031 | Delete Tasting from Edit Flow | Tasting | Low | v1.0.8 | Low | Low | 💡 Idea | Add Delete option when tapping Edit on a tasting — action sheet with Edit, Delete, Cancel instead of going straight to edit form |
| F032 | Log Again from Previous Tasting | Tasting | Medium | v1.0.8 | Low | Low | 💡 Idea | "Log again" shortcut on tastings with 2+ records — prompts user to use previous ratings or start fresh, prepopulates whiskey details, defaults to today's date |
