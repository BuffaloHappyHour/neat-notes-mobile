...
function buildEmailHtml(p: SemiActiveParams): string {
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#1F1F1F;border:1px solid rgba(190,150,99,0.16);border-radius:12px;overflow:hidden;">
          ...
          <tr>
            <td style="padding:32px 36px 36px;">
              ...
              <!-- Subheading -->
              <p style="font-family:Georgia,serif;font-size:17px;font-style:italic;color:#BE9663;margin:0 0 24px;">it has been a while.</p>

              <!-- Body copy -->
              <p style="font-family:'Helvetica Neue',Arial,sans-serif;font-size:14px;color:#DDD6CA;line-height:1.8;margin:0 0 24px;">You logged ${tastingCount} whiskies and were building something real. Your palate keeps coming back to ${topFlavor} notes -- and your best pour so far was ${topWhiskey} at ${topRating} points. That kind of taste memory does not fade.</p>
              ...
              <!-- Second paragraph -->
              <p style="font-family:'Helvetica Neue',Arial,sans-serif;font-size:14px;color:#DDD6CA;line-height:1.8;margin:0 0 24px;">Your palate profile is still here, waiting for your next pour. Log one whiskey and see where things stand. Premium shows you the full picture of everything you have built so far.</p>
              ...
              <table cellpadding="0" cellspacing="0" style="margin:0 0 16px;">
                <tr>
                  <td style="background:#BE9663;border-radius:8px;">
                    <a href="https://neatnotesapp.com/open?route=insights&utm_source=email&utm_medium=semi_active&utm_campaign=reengagement" style="display:inline-block;padding:14px 28px;color:#151515;font-family:'Helvetica Neue',Arial,sans-serif;font-size:14px;font-weight:600;text-decoration:none;">Pick up where you left off</a>
                  </td>
                </tr>
              </table>
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // ── 2. Users with >= 3 tastings, last_tasting_at in 31-60 day window ──────

    const since31d = new Date(Date.now() - 31 * 24 * 60 * 60 * 1000).toISOString();
    ...
    const metricRows = await rest(
      `user_metrics_lifetime_current?select=user_id,tasting_count,last_tasting_at&tasting_count=gte.3&last_tasting_at=lte.${since31d}&last_tasting_at=gte.${since60d}`
    ) as MetricRow[];
    ...
    for (const [id, profile] of profileMap) {
      ...
      candidates.push({
        ...
        segment: "email_semi_active",
      });
    }

    if (candidates.length === 0) {
      console.log("send-semi-active-email: no eligible candidates");
      return new Response(
    ...
    const sentRows = await rest(
      `sent_behavioral_notifications?select=user_id,sent_at&user_id=in.(${candidateIds.join(",")})&segment=eq.email_semi_active&order=sent_at.desc`
    ) as SentRow[];
    ...
    if (toSend.length > 0) {
      ...
      if (freqCapSkipped > 0) console.log(`send-semi-active-email: global_freq_cap_skipped=${freqCapSkipped}`);
    }

    if (toSend.length === 0) {
      console.log(`send-semi-active-email: all ${skipped} candidates already contacted`);
      return new Response(
      ...
      try {
        const res = await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            ...
            to: [email],
            subject: `You've logged ${candidate.tastingCount} whiskies. Your palate misses you.`,
            html: buildEmailHtml({
    ...
    console.log(`send-semi-active-email complete sent=${sent} skipped=${skipped} failed=${failed}`);
